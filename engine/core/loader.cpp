...
namespace VGED::Engine::HotLoader {
    ...
    template <typename T>
    Result<T> SharedLibrary::get_function(std::string name) {
        auto f = (T)dlsym(dlptr_, name.c_str());
        if (f)
            return f;
        else
            return ResultErr{ .message = { "Couldn't get a function" } };
    }
    ...
    int Executable::run(std::vector<std::string> args) {
        int (*main)(int, char**) =
            lib_->get_function<int (*)(int, char**)>(entry_point_).value(); // TODO: Expect fail
