...
void ScriptManager::load_script(const std::string& path) {
    ...
    auto func = lib.get_function<ScriptCreateFunctionPointer>("engine_get_script_object")
        .expect("Failed to find function in script"); // TODO handle

    ScriptObject script{func, lib, path};

    script_functions_.insert(std::pair<std::string, ScriptObject>(path, script));
}
...
void ScriptNameRegistry::register_name(
