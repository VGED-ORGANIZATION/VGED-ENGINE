...
Image::Image(
    ...
    device{_device}, description{_description}  {
    ...
    if (vkCreateImage(device->device(), &vk_image_create_info, nullptr, &vk_image)) {
        throw std::runtime_error("failed to create image!");
    }

    VkMemoryRequirements memory_requirements;
    vkGetImageMemoryRequirements(device->device(), vk_image, &memory_requirements);
    ...
    if (vkAllocateMemory(device->device(), &memory_allocate_info, nullptr, &vk_device_memory)) {
        throw std::runtime_error("failed to allocate image memory!");
    }

    if (vkBindImageMemory(device->device(), vk_image, vk_device_memory, 0)) {
...
Sampler::Sampler(std::shared_ptr<Device> _device, const SamplerDescription& _description) : device{_device}, description{_description} {
    ...
    if(vkCreateSampler(device->device(), &vk_sampler_create_info, nullptr, &vk_sampler)) {
        throw std::runtime_error("Failed to create sampler");
    }
}

Sampler::~Sampler() {
    vkDestroySampler(device->device(), vk_sampler, nullptr);
}
...
ImageView::ImageView(std::shared_ptr<Device> _device, const ImageViewDescription &_description) : device{_device}, description{_description} {
    ...
    if(vkCreateImageView(device->device(), &vk_image_view_create_info, nullptr, &vk_image_view)) {
